<template>
    ...
    <div id="list">
        <!-- table title -->
        <div
            class="uk-card uk-card-default uk-card-body tabsBorder"
            id="listTitle"
        >
        ...
        >
            <ul class="uk-flex-left" uk-tab id="tabNavUl">
                ...
                <li>
                    <a href="#" @click="tabsClick('chinese')"
                        >Chinese Translations</a
                    >
                </li>
                <li>
                    <a href="#" @click="tabsClick('korean')"
                        >Korean Translations</a
                    >
                </li>
                <li>
                    <a href="#" @click="tabsClick('japanese')"
                        >Japanese Translations</a
                    >
                </li>
                <li>
                    <a href="#" @click="tabsClick('webnovel')"
                        >Webnovel Originals</a
                    >
                </li>
                <li>
                ...
                <li>
                    <a href="#" @click="tabsClickDropped()"
                        >Stopped Reading (Dropped)</a
                    >
                </li>
            </ul>
        </div>
            ...
            <template slot="table-row" slot-scope="props">
                ...
                <span v-if="props.column.field == 'Description'">
                    ...
                    <div v-if="lengthChecker(props.row.Description)[0]">
                        <h4>Description</h4>
                        <p class="tableP fontColorChange">
                ...
                <span v-if="props.column.field == 'Dropped'">
                    <span
                        ...
                        >Dropped</span
                    >
                    <span class="uk-label tableLabel">{{
                        props.row.Origin
                    }}</span>
                </span>
                <!-- origin, hidden -->
        ...
        >
            ...
            <template slot="table-row" slot-scope="props">
                ...
                <span v-if="props.column.field == 'Title'">
                    ...
                    <div
                        class="uk-flex uk-flex-between uk-flex-middle mobileFlexChange"
                    >
                        <div>
                            <h4 class="noMargin">
                                Ranking: {{ props.row.Rank }} / 10
                            </h4>
                        </div>
                        <div>
                            <Modal :novel="props.row" uk-modal />
...
export default {
    ...
    methods: {
        ...
        //returns an array with the boolean if description is shorter than the determined char count
        //and returns that count
        //also checks if the description is not cuttable at a sentence end and instead should be displayed in full
        lengthChecker(input) {
            let width = Math.max(
            ...
            //determines the location of a sentence end at the location of the preview cut
            let cutOfPoint = input.indexOf(
                ". ",
                division - this.previewPositionCorrection
            );

            if (input.length < division || cutOfPoint == -1) {
        ...
        preview(input) {
            //cutting position
            let lengthcheck =
                this.lengthChecker(input)[1] - this.previewPositionCorrection;
            return input.slice(0, input.indexOf(". ", lengthcheck) + 2);
        },
        //returns the rest of the description without the preview portion
        afterPreview(input) {
            //cutting position
            let lengthcheck =
                this.lengthChecker(input)[1] - this.previewPositionCorrection;
            return input.slice(input.indexOf(". ", lengthcheck) + 2);
        },
